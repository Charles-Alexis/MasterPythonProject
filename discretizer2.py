...
class GridDynamicSystem:
    ...
    ############################
    def __init__(self, sys , x_grid_dim = [ 101 , 101 ], u_grid_dim = [ 11 ] , dt = 0.05 , lookup = True, esperance = [[0,1.]], print_data = True):
        """
    ...
    def compute(self):
        ...
        if self.print_data: 
            ...
            print('Esperance:', self.esperance )
            print('Road: ', self.sys.road[-1])
            print('Number of node-action pairs:', self.nodes_n * self.actions_n * len(self.esperance))
            print('---------------------------------------------------')
        
        self.generate_nodes()
        self.generate_actions_2()
        
        if self.computelookuptable:
            ## TRYING TO GO FOR ESPERANCE CALCUL
            print('Compute xnext Table')
            self.compute_xnext_table_E()
            print('Compute action Table')
            self.compute_action_set_table()
            
    def find_buuged_states(self, plotting=False):
        self.bugged_states_pos = np.zeros(self.x_grid_dim)
        self.bugged_states_vit = np.zeros(self.x_grid_dim)
        self.bugged_states = np.zeros(self.x_grid_dim)
        for p in range(len(self.x_level[0])):
            for v in range(len(self.x_level[1])):
                position = self.x_level[0][p]
                vitesse = self.x_level[1][v]
                dx = self.sys.f([position,vitesse], [0,0], 0)
                next_state_pos = position + dx[0] * 0.1
                next_state_vit = vitesse + dx[1] * 0.1
                pos_near = self.sys.find_nearest(self.x_level[0], next_state_pos)
                vit_near = self.sys.find_nearest(self.x_level[1], next_state_vit)
                # print(pos_near)
                # print(position)
                # print(vit_near)
                # print(vitesse)
        ...
        if plotting:        
            ...
            plt.ion()
            i = axs[0].pcolormesh(self.x_level[0], self.x_level[1], self.bugged_states_pos.T, shading='gouraud', cmap = 'plasma')
            axs[0].axis([self.x_level[0][0], self.x_level[0][-1], self.x_level[1][0], self.x_level[1][-1]])
            fig.colorbar(i, ax=axs[0])
    
            i = axs[1].pcolormesh(self.x_level[0], self.x_level[1], self.bugged_states_vit.T, shading='gouraud', cmap = 'plasma')
            axs[1].axis([self.x_level[0][0], self.x_level[0][-1], self.x_level[1][0], self.x_level[1][-1]])
            fig.colorbar(i, ax=axs[1])
            
            i = axs[2].pcolormesh(self.x_level[0], self.x_level[1], self.bugged_states.T, shading='gouraud', cmap = 'plasma')
            axs[2].axis([self.x_level[0][0], self.x_level[0][-1], self.x_level[1][0], self.x_level[1][-1]])
    ...
    #############################
    def discretize_input_space_2(self):
        """ Grid the input space """
        ...
        #### POUR SEULEMENT AVOIR LE NOMBRE DE SLIP DIFF
        self.actions_n = self.u_grid_dim[0]
        # range and step size for each dim
        self.u_range     = self.sys.u_ub - self.sys.u_lb
        self.u_step_size = self.u_range / ( self.u_grid_dim - 1 )
    ...
    def generate_actions_2(self):
        ...
        # 1-D List of actions
        self.input_from_action_id = np.zeros(self.u_grid_dim, dtype = float )  # Number of actions x inputs dimensions
        self.index_from_action_id = np.zeros(self.u_grid_dim, dtype = int   )  # Number of actions x inputs dimensions
        
        # For all state nodes
        action_id = 0
        
        # Single input
        # for u in np.flip(self.u_level[0]):
        for u in self.u_level[0]:
            ### SLIP
            self.input_from_action_id[action_id][0] = u
            self.index_from_action_id[action_id][0] = action_id
            ### OVERRIDE
            if self.input_from_action_id[action_id][0] == 0:
                 self.input_from_action_id[action_id][1] = 0
                 self.index_from_action_id[action_id][1] = 0
            else:
                 self.input_from_action_id[action_id][1] = 1
                 self.index_from_action_id[action_id][1] = 1
    ...
    ##############################
    def compute_xnext_table_E(self):
        """ Compute a x_next lookup table for the forward dynamics """
        ...
        # Evaluation lookup tables
        self.x_next_table = np.zeros( ( self.nodes_n , self.actions_n , len(self.esperance) , self.sys.n ) , dtype = float ) # lookup table for dynamic
        self.x_next_isok  = np.zeros( ( self.nodes_n , self.actions_n, len(self.esperance) ) , dtype = bool )
        ...
        for node_id in range( self.nodes_n ):  
                ...
                for action_id in range( self.actions_n ):
                    ...
                    for esp_id in range(len(self.esperance)):
                        ...
                        # Compute next state for all inputs
                        x_next = self.sys.f( x , u , e = self.esperance[esp_id][0] ) * self.dt + x
                        
                        if x_next[0] > self.sys.x_ub[0]:
                            x_next[0] = self.sys.x_ub[0]
                        if x_next[0] < self.sys.x_lb[0]:
                            x_next[0] = self.sys.x_lb[0]

                        if x_next[1] > self.sys.x_ub[1]:
                            x_next[1] = self.sys.x_ub[1]
                        if x_next[1] < self.sys.x_lb[1]:
                            x_next[1] = self.sys.x_lb[1]
                        
                        x_ok   = self.sys.isavalidstate( x_next )
                        
                        self.x_next_table[ node_id , action_id , esp_id , : ] = x_next
                        self.x_next_isok[  node_id , action_id , esp_id ]     = x_ok
    ...
    ##############################
    def plot_grid_values(self, J, J_confort, J_override, J_security, name = 'Value on the grid' , x = 0 , y = 1, jmax =  np.inf , jmin = 0, cmap = 'plasma'):   
        ##################################
        # Figure init
        ##################################
        ...
        axs[1].set_title('Confort')
        axs[2].set_title('Override')
        axs[3].set_title('Security')
        ##################################
        ### Create grid of data and plot
        #################################
        J_grid_nd = self.get_grid_from_array( J ) 
        J_grid_nd_confort = self.get_grid_from_array( J_confort ) 
        J_grid_nd_override = self.get_grid_from_array( J_override ) 
        J_grid_nd_security = self.get_grid_from_array( J_security ) 
        
        J_grid_2d = self.get_2D_slice_of_grid( J_grid_nd , x , y )
        J_grid_2d_confort = self.get_2D_slice_of_grid( J_grid_nd_confort , x , y )
        J_grid_2d_override = self.get_2D_slice_of_grid( J_grid_nd_override , x , y )
        J_grid_2d_security = self.get_2D_slice_of_grid( J_grid_nd_security , x , y )
        
        mesh = axs[0].pcolormesh( x_level, y_level, J_grid_2d.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        mesh_confort = axs[1].pcolormesh( x_level, y_level, J_grid_2d_confort.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        mesh_override = axs[2].pcolormesh( x_level, y_level, J_grid_2d_override.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        mesh_security = axs[3].pcolormesh( x_level, y_level, J_grid_2d_security.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        ...
        return fig, axs, mesh, mesh_confort, mesh_override, mesh_security           

    ##############################
    def plot_every_values(self, pi, J, J_confort, J_override, J_security, name = 'Value on the grid' , x = 0 , y = 1, jmax =  np.inf , jmin = 0, cmap = 'plasma'):   
        ##################################
        # Figure init
        ##################################
        ...
        axs[3].set_title('Override')
        axs[4].set_title('Security')
        ##################################
        ### Create grid of data and plot
        #################################
        pi_grid_nd = self.get_grid_from_array(self.get_input_from_policy( pi, 0 ))
        J_grid_nd = self.get_grid_from_array( J ) 
        J_grid_nd_confort = self.get_grid_from_array( J_confort ) 
        J_grid_nd_override = self.get_grid_from_array( J_override ) 
        J_grid_nd_security = self.get_grid_from_array( J_security ) 
        
        pi_grid_2d = self.get_2D_slice_of_grid( pi_grid_nd , x , y )
        J_grid_2d = self.get_2D_slice_of_grid( J_grid_nd , x , y )
        J_grid_2d_confort = self.get_2D_slice_of_grid( J_grid_nd_confort , x , y )
        J_grid_2d_override = self.get_2D_slice_of_grid( J_grid_nd_override , x , y )
        J_grid_2d_security = self.get_2D_slice_of_grid( J_grid_nd_security , x , y )
        
        mesh_pi = axs[0].pcolormesh( x_level, y_level, pi_grid_2d.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        mesh = axs[1].pcolormesh( x_level, y_level, J_grid_2d.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        mesh_confort = axs[2].pcolormesh( x_level, y_level, J_grid_2d_confort.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        mesh_override = axs[3].pcolormesh( x_level, y_level, J_grid_2d_override.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        mesh_security = axs[4].pcolormesh( x_level, y_level, J_grid_2d_security.T, shading='gouraud' , cmap = cmap) #, norm = colors.LogNorm()
        ...
        return fig, axs, mesh_pi, mesh, mesh_confort, mesh_override, mesh_security  
