...
roads = ['AsphalteDry', 'CementDry', 'AsphalteWet', 'CobblestoneWet', 'Snow']
...
sys.road = sys.roads[roads[4]]
slip_data = sys.return_max_mu()
sys.dmax = sys.f([0,sys.x_ub[1]],[-slip_data[1],1])


### DRIVER SETUP
tm_arr = [2.1, 2.2, 2.3, 2.8, 4.0]
sys.tm = 2.5
sys.tm_dot = 0.5
sys.tf = 1.75

timing_conservateur = -0.3
timing_normal = +0.0
timing_aggressif = +0.3
...
u_dim = [10,2]
x_dim = [300,150]
x_dims = [[300,80],[300,100],[300,150],[550,275],[550,600]]
x_lbs = [[-10., 0],[-10., 0],[-10., 0],[-15., 0],[-15., 0]]

x_dims = [[300,80],[300,80],[300,100],[300,150],[550,275]]
x_lbs = [[-10., 0],[-10., 0],[-10., 0],[-10., 0],[-15., 0]]
...
for r in range(len(roads)):
    sys.road = sys.roads[roads[r]]
    sys.tm = tm_arr[r]
    sys.x_grid = x_dims[r]
    sys.x_lb = np.array(x_lbs[r])
    ...
    name_list.append(roads[r]+'_'+E_arr_name[0])
    ...
    ttc_list.append(np.copy(ttc_controller.c_array()))
...
if plotting_flag:
    for d in range(len(roads)):
        ...
        u_vi = grid_sys_list[d].get_grid_from_array(grid_sys_list[d].get_input_from_policy(dp_list[d].pi, 0)).T
        u_ttc = ttc_list[d].T
        ...
        i = axs[0][0].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], u_vi, shading='gouraud')
        axs[0][0].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[0][0])
        axs[0][0].grid(True)
       
        axs[0][1].set_title('Cost2go')
        i = axs[0][1].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_tot, shading='gouraud')
        axs[0][1].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[0][1])
        axs[0][1].grid(True)
        
        axs[0][2].set_title('Confort Coef: '+str(cf.confort_coef))
        i = axs[0][2].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_confort, shading='gouraud')
        axs[0][2].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[0][2])
        axs[0][2].grid(True)  
        
        axs[0][3].set_title('Security Coef: ' + str(cf.security_coef))
        i = axs[0][3].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_security, shading='gouraud')
        axs[0][3].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[0][3])
        axs[0][3].grid(True)        
        
        axs[0][4].set_title('Override Coef: '+str(cf.override_coef))
        i = axs[0][4].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_override, shading='gouraud')
        axs[0][4].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[0][4])
        axs[0][4].grid(True)
        
        i = axs[1][0].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], u_ttc, shading='gouraud')
        axs[1][0].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[1, 0])
        axs[1][0].grid(True)

        i = axs[1][1].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_ttc_tot, shading='gouraud')
        axs[1][1].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[1, 1])
        axs[1][1].grid(True)
        
        i = axs[1][2].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_ttc_confort, shading='gouraud')
        axs[1][2].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[1, 2])
        axs[1][2].grid(True)  
        
        i = axs[1][3].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_ttc_security, shading='gouraud')
        axs[1][3].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[1, 3])
        axs[1][3].grid(True)        
        
        i = axs[1][4].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_ttc_override, shading='gouraud')
        axs[1][4].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[1 ,4])
        axs[1][4].grid(True)        

        i = axs[2][0].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], u_human, shading='gouraud')
        axs[2][0].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[2, 0])
        axs[2][0].grid(True)

        i = axs[2][1].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_tot_diff, shading='gouraud')
        axs[2][1].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[2, 1])
        axs[2][1].grid(True)
        
        i = axs[2][2].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_confort_diff, shading='gouraud')
        axs[2][2].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[2, 2])
        axs[2][2].grid(True)  
        
        i = axs[2][3].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_security_diff, shading='gouraud')
        axs[2][3].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
        fig.colorbar(i, ax=axs[2, 3])
        axs[2][3].grid(True)        
        
        i = axs[2][4].pcolormesh(grid_sys_list[d].x_level[0], grid_sys_list[d].x_level[1], c2g_override_diff, shading='gouraud')
        axs[2][4].axis([grid_sys_list[d].x_level[0][0], grid_sys_list[d].x_level[0][-1], grid_sys_list[d].x_level[1][0], grid_sys_list[d].x_level[1][-1]])
