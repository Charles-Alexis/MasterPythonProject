...
class DynamicProgramming:
    ...
    ################################
    def compute_steps(self, n = 50 , animate_cost2go = False , animate_policy = False , k = 0, treshhold = 0.01):
        """ compute number of step """
        
        print('\nComputing %d backward DP iterations:'%n)
        print('-----------------------------------------')
        
        if animate_cost2go: self.plot_cost2go(jmax=32000)
        ...
        for i in range(n):
            if flag is not True:
                ...
                dmax = self.finalize_backward_step_pourcent()
                if dmax < treshhold:
                    flag = True
...
class DynamicProgrammingWithLookUpTable( DynamicProgramming ):
    ...
    ############################
    def __init__(self, grid_sys , cost_function , final_time = 0, compute_cost = True):
...
class PolicyEvaluatorWithLookUpTable( PolicyEvaluator ):
    ...
    def compute_lookuptable(self):
        ...
        start_time = time.time()
        print('Computing g(x,u,t) and X-next look-up table..  ')
