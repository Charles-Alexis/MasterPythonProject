        ...
        for node in range( self.grid_sys.nodes_n ):  
                ...
                for action in range( self.grid_sys.actions_n ):
                    ...
                    if self.uselookuptable:
                        ...
                        if (u[1] is 0 and u[0] is not 0) or (u[1] is 1 and u[0] is 0):
                          u_ok = False
...
class ValueIteration_ND:
        ...
        for node in range(self.grid_sys.nodes_n):
            ...
            for action in range(self.grid_sys.actions_n):
                ...
                if action_isok:
                    ...
                    # Not allowable states or inputs/states combinations
                    Q[action] = self.cf.g(x, u, y, 0) * self.grid_sys.dt + ((x[1] + (dx[1]*self.grid_sys.dt))**2)
    ...
    def vi_law_baseline(self, x, t=0):
        """ Baseline controller from optimal actions """
        u = np.zeros(self.sys.m)
    ...
    def  compute_steps(self, l=50, plot=False, threshold=1.0e-25, maxJ=1000):
        ...
        threshold_flag = True    
        while step < l and threshold_flag is True:
            old_treshold = cur_threshold
            step = step + 1
            print('Step:', step)
            cur_threshold = self.compute_step()
            print('Current threshold', cur_threshold)
            if np.abs(cur_threshold) < self.threshold or (np.abs(old_treshold)-np.abs(cur_threshold))<0.001:
