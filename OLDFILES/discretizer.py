...
class GridDynamicSystem_w:
    """ Create a discrete gird state-action space for a continous dynamic system """
    
    ############################
    def __init__(self, sys , xgriddim = ( 101 , 101 ), ugriddim = ( 11 , 1 ) , wgriddim = ( 4 , 2 ), dt = 0.05 ):
        ...
        self.wgriddim = wgriddim
        
        self.wp = np.zeros(self.wgriddim)
    ...
    ##############################
    def generate_w(self,wp):
        """ Compute 1-D list of w """
        
        # For all state nodes
        w = 0
        
        # Single input 
        for k in range(self.wgriddim[0]):
            # State and grid index based on node #
            self.wp[w] = wp[w]
